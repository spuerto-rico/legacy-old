      ...
      <View style={{flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'flex-end'}}>
        <View>
          <View style={{flexDirection: 'row', justifyContent: 'flex-start', alignItems:'center'}}>
            <TouchableOpacity onPress={()=>this.props.onPressArrow('up', data.id)}>
              <Ionicons name="md-arrow-dropup" color="black" size={40} />
            </TouchableOpacity>
            <TouchableOpacity onPress={()=>this.props.onPressArrow('down', data.id)}>
              <Ionicons name="md-arrow-dropdown" color="black" size={40} />
            </TouchableOpacity>
            <Text style={{fontSize: 16, fontWeight: 'bold', marginLeft: 5}}>{data.title}</Text>
          </View>
          <Image source={{uri: data.content.thumbnail}} style={{height: 150, width: 200, resizeMode: 'contain'}} />
        </View>
        ...
        <Button
          ...
          onPress={() => {}}
        />
        <TouchableOpacity style={{position: 'absolute', top: -10, right: -5}} onPress={()=>this.props.onPressDelete(data.id)}>
    ...
    return (
      <View>
        <View style={{flexDirection: 'row', justifyContent: 'flex-start', alignItems:'center'}}>
          ...
          <Text style={{fontSize: 14, fontWeight: 'bold', marginBottom: 10, marginLeft: 5}}>{data.title}</Text>
        </View>
        <VideoPlayer
          endWithThumbnail
...
class SendInvites extends Component {
  ...
  componentDidMount() {
    AsyncStorage.getItem("profileInfo").then((res) => {
      ...
      console.log($res)
      this.setState({synergy_id: $res.synergy_id, userId: $res.id, purl: $res.purl});
    })
  ...
  onSetParams = async () => {
    ...
    var PurifyKitBuyButton = "<div class='card-body'>\
    <div class='row'>\
    <div class='col-8 col-sm-6' id='productLeft'>\
      <p><h4 style='font-size: 21px !important;'>PURIFY KIT</h4> <br>\
          <a href='https://"+this.state.synergy_id+"synergyworldwide.com/en-us/shop/product/Purify%20Kit' target='_blank'><button type='button' id='sharedbuttonproduct' value='1'>Buy</button> </a></p>\
        </div>\
          <div class='col-4 col-sm-6' id='productRight'>\
    ...
    var TrulumPackBuyButton = "<div class='card-body'>\
    <div class='row'>\
    <div class='col-8 col-sm-6' id='productLeft'>\
      <p><h4 style='font-size: 21px !important;'>TRULŪM PACK</h4> <br>\
          <a href='https://"+this.state.synergy_id+"synergyworldwide.com/en-us/shop/product/Trulūm%20Pack' target='_blank'><button type='button' id='sharedbuttonproduct' value='1'>Buy</button> </a></p>\
        </div>\
          <div class='col-4 col-sm-6' id='productRight'>\
  ...
  onNextStep = () => {
    if (this.state.type == "sms") {
      ...
      if (this.state.name == '' || this.state.message == '' || this.state.subject == '' || this.state.email == '') {
        alert('Please fill out all required fields');
        this.setState({
          isNullName: this.state.name == '' ? true : false,
          isNullMessage: this.state.message == '' ? true : false,
  ...
  onAddInvite = (data, id) => {
    ...
    this.setState({listCategoryInvite: $listInvitePreview, isCategoryVisible: false});
  }
  ...
  onReOrder = (direction, hashId) => {
    ...
    order.splice(direction == 'down' ? indexOfElement + 1 : indexOfElement - 1, 0, order.splice(indexOfElement, 1)[0]);
    this.setState({listCategoryInvite: $listInvitePreview});
  }
  ...
  renderCategory = () => {
    return (
      ...
      >
        <View style={{flex: 1}}>
          <View style={{height: 120, backgroundColor: '#00acef'}}>
            <View style={{paddingTop: 60, paddingHorizontal: 20}}>
              <Text style={{fontSize: 18, fontWeight: 'bold', color: 'white'}}>Select which info you want to add to the landing page that will be shared.</Text>
            </View>
          </View> 
          <ScrollView style={{flex: 1}} contentContainerStyle={{padding: 20}}>
            {
              Data.category.map((item, id) => {
                return (
                  <AccordionCategory key={id} title={item.title} contentList={item} onPress={(data, key)=>this.onAddInvite(data, key)} />
                )
              })
            }
          </ScrollView>
        ...
        <TouchableOpacity style={styles.rightPosition} onPress={()=>this.setState({isCategoryVisible: false})}>
          <Ionicons name="ios-close" color="black" size={40} />
        </TouchableOpacity>
      </Modal>
    )
  ...
  renderPreviewInvite = () => {
    return (
      ...
      >
        <View style={{flex: 1}}>
          <View style={{height: 100, backgroundColor: '#00acef'}}>
            <View style={{flex: 1, alignItems: 'center', justifyContent: 'center', paddingTop: 40}}>
              <Text style={{fontSize: 18, fontWeight: 'bold', color: 'white'}}>INVITE PREVIEW</Text>
            </View>
            <View style={styles.rightPosition}>
              <Button
                  ...
                  buttonStyle={{width: 80, height: 40, backgroundColor: 'tranparent'}}
                  onPress={() => this.onSendInvite()}/>
            </View>
          </View>
          <Image source={{uri: 'http://dev.legacynetwork.com/files/logo.png'}} style={{width: 220, height: 60, resizeMode: 'contain', alignSelf: 'center', marginTop: 10}} />
          <View style={{flex: 1}}>
            <SortableListView
              style={{ flex: 1 }}
              ...
              renderRow={(row, index) => <RowComponent data={row} onPressDelete={(hashId)=>this.onDeleteItem(hashId)} onPressArrow={(direction, hashId) =>this.onReOrder(direction, hashId)} />}
            />
        ...
        {this.renderCategory()}
        <TouchableOpacity style={styles.lefttPosition} onPress={()=>this.setState({isPreviewInvite: false})}>
  ...
  render () {
    ...
    return (
      <View style={{flex: 1}}>
      <AppHeader title="SEND INVITES" onLeftButtonPress={() => this.props.navigation.openDrawer()} />
         ...
         <Card title="SEND INVITES" isShadow={true}>
           <View style={styles.textContainer}>
             ...
             <FormInline label="Message Type" style={{zIndex: 10}}>
              <DropDownPicker
                items={[
                    {label: 'SMS Message', value: 'sms', icon: () => <Ionicons name="ios-chatbubbles" size={18} color="black" />},
                    {label: 'Email', value: 'email', icon: () => <Ionicons name="ios-mail" size={18} color="black" />},
                ]}
                ...
                dropDownStyle={{backgroundColor: 'white', marginTop: 2}}
                ...
                <FormInline label="Name" style={{zIndex: 10}}>
                  {/* <Input
                    ...
                    containerStyle={{marginBottom: 10}}/>
                    {
                      ...
                      <Text style={styles.errorText}>*Please fill out this required field</Text>
                      :
                      null
                    } */}
                  <Autocomplete
                    autoCapitalize="none"
                    autoCorrect={false}
                    data={contacts.length === 1 && comp(query, contacts[0].givenName) ? [] : contacts}
                    defaultValue={query}
                    onChangeText={text => this.setState({ query: text, name: text })}
                    placeholder=""
                    renderItem={(res, id) => {
                      return (
                        <TouchableOpacity style={{backgroundColor: 'white', paddingVertical: 5}} key={id} onPress={() => this.setState({query: res.item.givenName, name: res.item.givenName, phone: res.item.phoneNumbers.length > 0 ? res.item.phoneNumbers[0].number : 'None'})}>
                          <Text style={{paddingVertical: 5, paddingHorizontal: 5}}>
                            {res.item.givenName} {res.item.phoneNumbers.length > 0 ? res.item.phoneNumbers[0].number : 'None'}
                          </Text>
                        </TouchableOpacity>
                      )
                    }}
